...
class IntParticipantCommittee extends Model
{
    protected $table = 't_int_committee_participant';
    protected $perPage = 10;
	public $timestamps = true;

    protected $fillable = [
		'id_internal_committee', 'id_user', 'initial_period', 'final_period', 'id_level', 'id_role', 'score', 'role_description', 'approve', 'na', 'created_at', 'created_by', 'updated_at', 'updated_by'
    ];

    public static function baseQuery_seminar()
	{
		return DB::table('t_int_committee_participant AS cp')
        ->join('m_level_seminar AS l', 'cp.id_level', 'l.id')
        ->join('m_role_seminar AS r', 'cp.id_role', 'r.id')
		->join('m_users AS u', 'u.nim_nik', 'cp.id_user')
		->select('cp.*', 'l.name AS level_name', 'r.name AS role_name', 'u.name AS student_name');
	}

	public static function baseQuery_competition()
	{
		return DB::table('t_int_committee_participant AS cp')
        ->join('m_level_competition AS l', 'cp.id_level', 'l.id')
        ->join('m_role_competition AS r', 'cp.id_role', 'r.id')
		->join('m_users AS u', 'u.nim_nik', 'cp.id_user')
		->select('cp.*', 'l.name AS level_name', 'r.name AS role_name', 'u.name AS student_name');
	}
