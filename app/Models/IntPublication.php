...
class IntPublication extends Model
{
    protected $table = 't_internal_publication';
	...
	{
		return DB::table('t_internal_publication AS cp')
	...
	{
		return DB::table('t_internal_publication')->select('*')->where('id', $id);
	}

	public static function getCPMember($id)
	{
		return DB::table('t_internal_publication AS cp')
		->join('t_int_publication_member AS m', 'm.id_internal_publication', 'cp.id')
