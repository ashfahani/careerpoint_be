...
class CPCommittee extends Model
{
    protected $table = 't_cp_committee';
    protected $perPage = 10;
	public $timestamps = true;

    protected $fillable = [
		'id_activity_type', 'id_activity_category', 'id_user', 'activity_name', 'initial_period', 'final_period', 'period', 'organizer_name', 'organizer_location', 'id_level', 'id_role', 'score', 'approve', 'na', 'file', 'file_type', 'reject_text', 'id_internal_committee', 'send_email', 'created_at', 'created_by', 'updated_at', 'updated_by'
    ];

    public static function baseQuery()
	{
		return DB::table('t_cp_committee AS cp')
        ->join('m_activity_type AS at', 'cp.id_activity_type', 'at.id')
        ->join('m_activity_category AS ac', 'cp.id_activity_category', 'ac.id')
        ->join('m_level_committee AS l', 'cp.id_level', 'l.id')
        ->join('m_role_committee AS r', 'cp.id_role', 'r.id')
