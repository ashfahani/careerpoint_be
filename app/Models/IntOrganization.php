...
class IntOrganization extends Model
{
    protected $table = 't_internal_organization';
	...
	{
		return DB::table('t_internal_organization AS cp')
	...
	{
		return DB::table('t_internal_organization')->select('*')->where('id', $id);
	}

	public static function getCPMember($id)
	{
		return DB::table('t_internal_organization AS cp')
		->join('t_int_organization_member AS m', 'm.id_internal_organization', 'cp.id')
