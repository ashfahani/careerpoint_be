...
class CPInternshipController extends Controller
{
    public function getCP(Request $request)
    {
        ...
        try {
            $data = CPInternship::baseQuery();
            if ($request->search){
                $data = CPInternship::searchFilter($data, $request->search);
            }
            $data = CPInternship::statusFilter($data, $request->status);
    ...
    public function getCPDetails($id)
    {
        try {
            ...
            $res = CPInternship::baseQuery()
                ->where('cp.id', '=', $id)
                ->get();
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Get Details CP Internship ID = '.$id,
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function add(Request $request)
    {
        $validation = Validator::make(
            ...
            [
                ...
                'id_level' => ['exists:m_level_internship,id'],
                'id_role' => ['exists:m_role_internship,id'],
        ...
        try {
            $l_score = MLevelInternship::getScore($request->id_level)->where('id_activity_category', '2')->first();
            ...
            $insert = CPInternship::insertGetId($data);
    ...
    public function update(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'id_cp' => ['required', 'exists:t_cp_internship,id'],
            'id_activity_type' => ['exists:m_activity_type,id'],
            'activity_name' => ['required', 'string', 'max:255'],
            'initial_period' => ['required', 'date_format:Y-m-d'],
            'final_period' => ['required', 'date_format:Y-m-d', 'after_or_equal:initial_period'],
        ...
        try {
            ...
            $affected = CPInternship::where('id', $request->id_cp)->update($data);
    ...
    {
        try {
            $affected = CPInternship::where('id', $id)->delete();

            if ($affected) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Delete CP Internship ID = '.$id,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    public function getCPbyMentor(Request $request)
    {
        ...
        try {
            $data = CPInternship::queryForMentor()->where('u.mentor', auth()->user()->nim_nik);
            if ($request->search){
                $data = CPInternship::searchFilter($data, $request->search);
            }
            $data = CPInternship::statusFilter($data, $request->status);
            ...
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Get CP Internship by Mentor',
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function approve(Request $request)
    {
        ...
        try {            
            ...
            if ($affected) {
                $getCP = CPInternship::queryForMentor()->where('cp.id', $request->id_cp)->first();
