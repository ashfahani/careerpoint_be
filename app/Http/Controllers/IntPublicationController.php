...
use App\Models\IntMemberPublication;
use App\Models\IntPublication;
use App\Models\Logs;
...
class IntPublicationController extends Controller
{
    public function getCP(Request $request)
    {
        ...
        try {
            $data = IntPublication::baseQuery();
            if ($request->search){
                $data = IntPublication::searchFilter($data, $request->search);
            }
            $data = IntPublication::statusFilter($data, $request->status);
            ...
            $logInfo = array(
                ...
                'activity' => 'Get CP Publication Internal',
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function getCPDetails($id)
    {
        try {
            ...
            $res = IntPublication::baseQuery()
                ->where('cp.id', '=', $id)
                ->get();
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Get Details CP Publication Internal ID = '.$id,
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            $insert = IntPublication::insertGetId($data);
    ...
    public function update(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'id_cp' => ['required', 'exists:t_internal_publication,id'],
            'id_activity_type' => ['exists:m_activity_type,id'],
            'activity_name' => ['required', 'string', 'max:255'],
            'initial_period' => ['required', 'date_format:Y-m-d'],
            'final_period' => ['required', 'date_format:Y-m-d', 'after_or_equal:initial_period'],
        ...
        try {
            ...
            $affected = IntPublication::where('id', $request->id_cp)->update($data);
            ...
            if ($affected) {
                ...
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Update CP Publication Internal ID = '.$request->id_cp,
                    'stat' => 'error'
                );
                Logs::create($logInfo);
    ...
    {
        try {
            $affected = IntPublication::where('id', $id)->delete();
            $affected2 = IntMemberPublication::where('id_internal_publication', $id)->delete();

            if ($affected>0 || $affected2>0) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Delete CP Publication Internal ID = '.$id,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    public function finalize(Request $request)
    {
        ...
        try {            
            ...
            if ($affected) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Finalize CP Publication by Mentor ID='.$request->id_cp,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    public function approve(Request $request)
    {
        ...
        try {          
            $listCP = IntPublication::getCPMember($request->id_cp);
            foreach($listCP as $r){
                $CPinfo = array(
                    "id_user"               => $r->id_user,
                    ...
                    "id_internal_publication" => $r->id,
                    "created_at"            => date("Y-m-d H:i:s"),
                    "created_by"            => auth()->user()->nim_nik
                );

                $insertInt = CPPublication::insertGetId($CPinfo);
            }  
            $data = array(
                'final' => 'N',
                'approve' => 'A',
            ...
            $affected2 = IntMemberPublication::where('id', $request->id_cp)->update($data2);
    ...
    public function getAllCPMember(Request $request)
    {
        ...
        try {
            $data = IntMemberPublication::baseQuery();
            if ($request->search){
                $data = IntMemberPublication::searchFilter($data, $request->search);
            }
            $data = IntMemberPublication::statusFilter($data, $request->status);
            $data = $data->where('cp.id_internal_publication', $request->id_cp);
            $data = $data->orderBy('cp.created_at', 'desc');
            $limit = $request->limit;
            if ($request->limit == 0)
                $limit = $data->get()->count();
            ...
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Get CP Publication Internal Member',
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function addMember(Request $request)
    {
        $validation = Validator::make($request->all(), [
            ...
            'id_level' => ['required', 'exists:m_level_publication,id'],
            'id_role' => ['required', 'exists:m_role_publication,id'],            
        ]);
        if ($validation->fails()) {
            return ResponseFormatter::error($validation->errors(), 'Validation Error!');
        }
        ...
        try {
            ...
            $CPDetail = IntPublication::getCPDetail($request->id_cp)->first();

            $data = array(
                'id_internal_publication' => $request->id_cp,
            ...
            $insert = IntMemberPublication::insertGetId($data);

            if ($insert) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Add Member of CP Publication ID = '.$request->id_cp,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
            ...
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Add Member CP Publication ID = '.$request->id_cp,
                'stat' => 'error'
            );
            Logs::create($logInfo);
            return ResponseFormatter::error([
    ...
    {
        $validation = Validator::make($request->all(), [
            'id_member' => ['required', 'exists:t_int_publication_member,id'],
            // 'id_cp' => ['required', 'exists:t_internal_publication,id'],
            'id_user' => ['required', 'exists:m_users,nim_nik'],
            'id_level' => ['required', 'exists:m_level_publication,id'],
            'id_role' => ['required', 'exists:m_role_publication,id'],            
        ...
        try {
            ...
            $affected = IntMemberPublication::where('id', $request->id_member)->update($data);

            if ($affected) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Updated Member of CP Publication ID = '.$request->id_cp,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
                ...
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Update Member of CP Publication ID = '.$request->id_cp,
                    'stat' => 'error'
                );
                Logs::create($logInfo);
            ...
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Update Member CP Publication ID = '.$request->id_cp,
                'stat' => 'error'
            );
            Logs::create($logInfo);
            return ResponseFormatter::error([
    ...
    {
        try {
            $affected = IntMemberPublication::where('id', $id)->delete();

            if ($affected) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Delete CP Publication Internal Member ID = '.$id,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
                return ResponseFormatter::success([
