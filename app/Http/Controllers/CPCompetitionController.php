...
class CPCompetitionController extends Controller
{
    public function getCP(Request $request)
    {
        ...
        try {
            $data = CPCompetition::baseQuery();
            if ($request->search){
                $data = CPCompetition::searchFilter($data, $request->search);
            }
            $data = CPCompetition::statusFilter($data, $request->status);
    ...
    public function getCPDetails($id)
    {
        try {
            ...
            $res = CPCompetition::baseQuery()
                ->where('cp.id', '=', $id)
                ->get();
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Get Details CP Competition ID = '.$id,
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function add(Request $request)
    {
        $validation = Validator::make(
            ...
            [
                ...
                'id_level' => ['exists:m_level_competition,id'],
                'id_role' => ['exists:m_role_competition,id'],
        ...
        try {
            $l_score = MLevelCompetition::getScore($request->id_level)->where('id_activity_category', '2')->first();
            ...
            if($request->hasFile('file')){
                ...
                $file_save = "Competition/".$file_name;
                $file_type = $file->getClientOriginalExtension();
                $file->storeAs('files/Competition', $file_name);
            }
            ...
            $insert = CPCompetition::insertGetId($data);

            if ($insert) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Add CP Competition ID = '.$insert,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    public function update(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'id_cp' => ['required', 'exists:t_cp_competition,id'],
            'id_activity_type' => ['exists:m_activity_type,id'],
            'activity_name' => ['required', 'string', 'max:255'],
            'initial_period' => ['required', 'date_format:Y-m-d'],
            'final_period' => ['required', 'date_format:Y-m-d', 'after_or_equal:initial_period'],
        ...
        try {
            ...
            $affected = CPCompetition::where('id', $request->id_cp)->update($data);

            if ($affected) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Update CP Competition ID = '.$request->id_cp,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    {
        try {
            $affected = CPCompetition::where('id', $id)->delete();

            if ($affected) {
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Delete CP Competition ID = '.$id,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    public function getCPbyMentor(Request $request)
    {
        ...
        try {
            $data = CPCompetition::queryForMentor()->where('u.mentor', auth()->user()->nim_nik);
            if ($request->search){
                $data = CPCompetition::searchFilter($data, $request->search);
            }
            $data = CPCompetition::statusFilter($data, $request->status);
            ...
            $logInfo = array(
                'user' => auth()->user()->nim_nik,
                'activity' => 'Get CP Competition by Mentor',
                'stat' => 'success'
            );
            Logs::create($logInfo);
    ...
    public function approve(Request $request)
    {
        ...
        try {            
            ...
            if ($affected) {
                $getCP = CPCompetition::queryForMentor()->where('cp.id', $request->id_cp)->first();
                ...
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Approve CP Competition by Mentor ID='.$request->id_cp,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
    ...
    public function reject(Request $request)
    {
        ...
        try {            
            ...
            if ($affected) {
                ...
                $logInfo = array(
                    'user' => auth()->user()->nim_nik,
                    'activity' => 'Reject CP Competition by Mentor ID='.$request->id_cp,
                    'stat' => 'success'
                );
                Logs::create($logInfo);
